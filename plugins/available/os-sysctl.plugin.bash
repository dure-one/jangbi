## \brief sysctl install configurations. <div style="text-align: right"> group:**prenet** | runtype:**none** | deps: **-** | port: **-**</div><br/>
## \desc 
## [Sysctl](https://man7.org/linux/man-pages/man8/sysctl.8.html){:target="_blank"} manages kernel runtime parameters for system optimization and security hardening.
# It provides automated configuration management for kernel parameters, allowing fine-tuning of system behavior
# including network settings, memory management, and security parameters.
## 
## ![sysctl configuration](../imgs/sysctl_1.png "sysctl configuration")
## # Jangbi Configs
...
## Current configuration is stored in `/etc/sysctl.d/`. it is generated by `os-sysctl configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/sysctl.d/99-disable-maxusernamespaces.conf"
## --8<-- "./configs/sysctl/99-disable-maxusernamespaces.conf"
## ```
## ```bash title="/etc/sysctl.d/99-disable-ipv6.conf"
## --8<-- "./configs/sysctl/99-disable-ipv6.conf"
## ```
## ```bash title="/etc/sysctl.d/99-disable-coredump.conf"
## --8<-- "./configs/sysctl/99-disable-coredump.conf"
## ```
## ```bash title="/etc/sysctl.d/98-mikehoen-sysctl.conf"
## --8<-- "./configs/sysctl/98-mikehoen-sysctl.conf"
## ```
## ```bash title="/etc/sysctl.d/98-imthenachoman-sysctl.conf"
## --8<-- "./configs/sysctl/98-imthenachoman-sysctl.conf"
## ```
## ```bash title="/etc/sysctl.d/98-2dure-sysctl.conf"
## --8<-- "./configs/sysctl/98-2dure-sysctl.conf"
## ```
