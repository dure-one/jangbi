## \brief dnsmasq install configurations. <div style="text-align: right"> group:**postnet** | runtype:**minmon** | deps: **-** | port: **LO:53**</div><br/>
## \desc 
## [Dnsmasq](https://thekelleys.org.uk/dnsmasq/doc.html){:target="_blank"} is a lightweight DNS forwarder and DHCP server for small networks.
# It provides network services including DNS caching, DHCP allocation, and network boot services.
# We can use it for DNS blacklist filtering and local network management.
## 
## 
## # Jangbi Configs
...
## Current configuration is stored in `/etc/dnsmasq/`. it is generated by `net-dnsmasq configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/dnsmasq/dnsmasq.conf"
## --8<-- "./configs/dnsmasq/dnsmasq.conf.sample"
## ```
