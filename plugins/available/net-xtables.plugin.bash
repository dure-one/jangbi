## \brief xtables extended netfilter configurations. <div style="text-align: right"> group:**net** | runtype:**systemd** | deps: **-** | port: **-**</div><br/>
## \desc 
## [Xtables](https://netfilter.org/projects/iptables/index.html){:target="_blank"} provides extended netfilter configurations for advanced packet filtering.
# It provides automated installation, configuration management, and extended iptables rule management.
# Xtables enables advanced firewall configurations with additional netfilter modules and capabilities.
## 
## # Jangbi Configs
...
## Current configuration is stored in `/etc/xtables/`. it is generated by `net-xtables configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/xtables/rulesfile.conf"
## --8<-- "./configs/xtables/rulesfile.conf.sample"
## ```
