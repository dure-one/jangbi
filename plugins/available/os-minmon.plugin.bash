...
## Current configuration is stored in `/etc/minmon/`. it is generated by `os-minmon configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/minmon/minmon.toml"
## --8<-- "./configs/minmon/minmon.toml.sample"
## ```
...
## \usage os-minmon install|uninstall|download|disable|configgen|configapply|check|run
