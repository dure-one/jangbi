...
function os-repos {
    ...
    if [[ $# -eq 1 ]] && [[ "$1" = "install" ]]; then
        ...
        __os-repos_enable_additional "$2"
    elif [[ $# -eq 1 ]] && [[ "$1" = "disableall" ]]; then
        __os-repos-disable_additional "$2"
    else
        __os-repos_help
    fi
}
...
function __os-repos-disable_offline {
...
function __os-repos_uninstall { # RUN_OS_FIRMWARE=0
...
function __os-repos_check { # running_status 0 installed, running_status 5 can install, running_status 10 can't install
    running_status=0
