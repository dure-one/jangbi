## \brief hysteria install configurations. <div style="text-align: right"> group:**postnet** | runtype:**minmon** | deps: **-** | port: **LO:1080**</div><br/>
## \desc 
## [Hysteria](https://v2.hysteria.network/){:target="_blank"} is a feature-packed proxy & relay tool optimized for lossy, unstable connections.
# It provides automated installation, configuration management, and service control capabilities.
# Hysteria offers brutal performance for high-performance proxy and tunneling services.
## 
## # Jangbi Configs
...
## Current configuration is stored in `/etc/hysteria/`. it is generated by `net-hysteria configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/hysteria/config.yaml"
