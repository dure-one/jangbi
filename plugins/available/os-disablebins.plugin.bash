## \brief disable system binaries for security. <div style="text-align: right"> group:**postos** | runtype:**manual** | deps: **-** | port: **-**</div><br/>
## \desc 
## This tool helps disable potentially dangerous system binaries for enhanced security hardening.
# It provides automated installation, configuration management, and binary restriction capabilities.
# The tool safely disables or replaces system binaries that could be used maliciously while
...
## Current configuration is stored in `/etc/disablebins/`. it is generated by `os-disablebins configgen` command on install.
## You can edit it manually and not run install or configapply commands to keep current configurations.
## ```bash title="/etc/disablebins/config"
## ```
...
## \usage os-disablebins install|uninstall|download|disable|configgen|configapply|check|run
...
function __os-disablebins_help {
    echo -e "Usage: os-disablebins [COMMAND]\n"
